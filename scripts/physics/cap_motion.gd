...
func integrate(velocity: Vector2, steering: float, delta: float, flow: Vector2 = Vector2.UP, external: Vector2 = Vector2.ZERO, speed_modifier: float = 1.0, acceleration_multiplier: float = 1.0) -> Vector2:
	var forward := flow.normalized()
	var lateral := Vector2(-forward.y, forward.x)
	var advance := velocity.dot(forward)
	advance = move_toward(advance, config.current_speed * speed_modifier, config.current_speed * config.acceleration * acceleration_multiplier * delta)
