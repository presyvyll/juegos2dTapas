...
foreach ($template in @('android_debug.apk', 'windows_debug_x86_64.exe', 'version.txt')) {
...
New-Item -ItemType Directory -Path 'builds/android','builds/windows','builds/logs' -Force | Out-Null
$engine = Join-Path $engineDir 'Godot_v4.3-stable_win64_console.exe'
foreach ($preset in @('Android', 'Windows Debug')) {
...
$artifacts = foreach ($relative in @('builds/android/tapa-racing.apk', 'builds/windows/TapaRacing.exe')) {
